...
function pushToQueue(_a) {
    ...
    for (var systemName in observerInfos) {
        ...
        var index = info.findIndex(function (p) {
            return isEqual__default['default'](p, prop);
        });
        if (index > -1) {
            addObserver({
...
function defineProperty(_a) {
    ...
    if (prop.deep && isObject__default['default'](obj[key])) {
        try {
    ...
    ComponentObserver.prototype.add = function (_a) {
        ...
        var index = this.events.findIndex(function (changed) {
            return changed.component === component &&
                isEqual__default['default'](changed.prop, prop) &&
                changed.type === type;
        });
        if (index > -1) {
...
resourceLoader.XhrLoadStrategy.setExtensionXhrType('json', resourceLoader.XhrResponseType.Json);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('tex', resourceLoader.XhrResponseType.Json);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('ske', resourceLoader.XhrResponseType.Json);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('mp3', resourceLoader.XhrResponseType.Buffer);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('wav', resourceLoader.XhrResponseType.Buffer);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('aac', resourceLoader.XhrResponseType.Buffer);
resourceLoader.XhrLoadStrategy.setExtensionXhrType('ogg', resourceLoader.XhrResponseType.Buffer);
var STRATEGY = {
    png: resourceLoader.ImageLoadStrategy,
    jpg: resourceLoader.ImageLoadStrategy,
    jpeg: resourceLoader.ImageLoadStrategy,
    webp: resourceLoader.ImageLoadStrategy,
    json: resourceLoader.XhrLoadStrategy,
...
var Resource = (function (_super) {
    ...
    Resource.prototype.getLoader = function (preload) {
        ...
        if (!loader) {
            loader = new resourceLoader.Loader();
